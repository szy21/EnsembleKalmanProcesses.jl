...
function construct_priors()
    ...
    params = Dict(
        ...
        # "detrainment_factor"        => [bounded(0.0, 2.0*0.31)],
...
function run_calibrate(return_ekobj=false)
    ...
    g_(x::Vector{FT}) where FT<:Real = run_SCM(
    ...
    for i in 1:N_iter
        ...
        array_of_tuples = [g_(params[j]) for j in 1:length(params)]
